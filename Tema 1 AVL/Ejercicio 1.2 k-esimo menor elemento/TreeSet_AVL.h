...
public:
   ...
   T const& kesimo(int k) const {
       ...
       Link a = raiz;
       while (a != nullptr) {
           int ti = getTam_i(a); // posición del nodo actual
   ...
   void printTam_i(Link a,int nivel = 0) {
       if (a == nullptr) return;
       printTam_i(a->dr, nivel + 1);
       std::cout << std::string(nivel * 2, ' ') << a->elem << " (tam_i=" << a->tam_i << ")\n";
       printTam_i(a->iz, nivel + 1);
   }
...
protected:
   ...
   int getTam_i(const Link& a) const{
       if (a == nullptr) return 0;
       else return a->tam_i;
   }
   ...
   void rotaDer(Link & r2) {
	  ...
	  r2->tam_i = getTam_i(r2) - getTam_i(r1);
      r2 = r1;
   }
